...
(define (ifoldr combine base lst)
  (ifoldr-h combine base lst 0)
  )
   

(define (ifoldr-h combine base lst n)
  (cond
    [(empty? lst) base]
    [else (combine n (first lst)
                   (ifoldr-h combine base (rest lst) (add1 n)))]
    )
  )
...
(define (remove-letters s)
  (local [(define loc (string->list s))]
    (build-list (length loc) (lambda (i)
                               (list->string (remove-at i loc))))))
...
(define (suggest s valid?)
  (local [(define words (append (remove-letters s)
          ...
          (define clean-words
            (remove-dups (sort legal-words string<=?)))]
